  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await api.generate({
        output_types: settings.output_types.map((o) => o.id),
        ...
        social_platform: settings.social_platforms[0]?.id || '',
        score: signal.score,
        tier: signal.tier,
        layers: signal.layers,
        active_lens_ids: activeLenses,
        input_mode_id: settings.input_modes[0]?.id || '',
        persona_ids: settings.personas.slice(0, 1).map((p) => p.id),
        pov_ids: settings.pov_options.slice(0, 1).map((p) => p.id),
        pov_custom: '',
        web_research: true,
